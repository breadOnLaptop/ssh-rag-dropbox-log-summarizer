...
sudo cp src/watcher.py "$INSTALL_DIR/src/"
sudo cp requirements.txt "$INSTALL_DIR/"
if [ ! -f "$INSTALL_DIR/config/user_keys.json" ]; then
    sudo cp config/user_keys.example.json "$INSTALL_DIR/config/user_keys.json"
fi
...
echo "Setup complete. Please configure $INSTALL_DIR/config/user_keys.json"
